...
internal class ExportLuaHelper
{
    ...
    public static StringBuilder GetI18n()
    {
        ...
        // foreach (var item in multLangueDataDic)
        // {
        //     AppendLineContentIndent(content, $"[\"{item.Key}\"] = {{");
        //     ++AppConfig.Indentlevel;
        //     AppendLineContentIndent(content, $"[\"zh\"] = \"{item.Key}\",");
        //     --AppConfig.Indentlevel;
        //     foreach (var value in item.Value.value)
        //     {
        //         ++AppConfig.Indentlevel;
        //         AppendLineContentIndent(content, $"[\"{value.Key}\"] = \"{value.Value}\",");
        //         --AppConfig.Indentlevel;
        //     }
        //     content.Remove(content.Length - 1, 1);
        //     AppendLineContentIndent(content, "},");
        ...
        // foreach (var item in varTranslations)
        // {
        //     AppendLineContentIndent(content, $"{item.Key} = multilingual[\"{item.Value}\"],");
        // }
        // content.Remove(content.Length - 1, 1);
        // content.AppendLine("}");
        // content.AppendLine();
        // //添加原表操作
        // content.AppendLine("return setmetatable({ language = \"en\" }, {");
